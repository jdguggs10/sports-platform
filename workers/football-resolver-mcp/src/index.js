/**
 * Football Entity Resolver MCP Worker
 * 
 * Lightweight LLM resolver for football player and team naming discrepancies.
 * Resolves fuzzy entity names to canonical IDs for MCP tool calls.
 * 
 * Features:
 * - NFL team name resolution with aliases
...
export default {
  async fetch(request, env) {
    try {
      ...
      return new Response('Football Entity Resolver MCP v1.0', {
...
async function handleHealthCheck(env) {
  try {
    ...
    const teamCount = await env.FOOTBALL_DB.prepare(
      'SELECT COUNT(*) as count FROM teams'
    ).first();
    
    const playerCount = await env.FOOTBALL_DB.prepare(
...
async function handleTeamSearch(body, env) {
  try {
    ...
    const teams = await env.FOOTBALL_DB.prepare(sql).bind(...params).all();
...
async function handlePlayerSearch(body, env) {
  try {
    ...
    const players = await env.FOOTBALL_DB.prepare(sql).bind(...params).all();
...
async function resolveTeam(name, env, options = {}) {
  try {
    ...
    let team = await env.FOOTBALL_DB.prepare(`
    ...
    // Try alias match
    if (!team) {
      team = await env.FOOTBALL_DB.prepare(`
    ...
    // Add aliases
    const aliases = await env.FOOTBALL_DB.prepare(`
    ...
    if (options.includeStats) {
      const stats = await env.FOOTBALL_DB.prepare(`
        SELECT 
...
async function resolvePlayer(name, env, options = {}) {
  try {
    ...
    let player = await env.FOOTBALL_DB.prepare(sql).bind(...params).first();
    ...
    if (!player) {
      ...
      player = await env.FOOTBALL_DB.prepare(sql).bind(...params).first();
    }
    ...
    if (options.includeStats) {
      const stats = await env.FOOTBALL_DB.prepare(`
        SELECT * FROM player_stats WHERE player_id = ? AND season = 2024
...
async function getTeamSuggestions(name, env, limit = 5) {
  try {
    const suggestions = await env.FOOTBALL_DB.prepare(`
...
async function getPlayerSuggestions(name, env, options = {}, limit = 5) {
  try {
    ...
    const suggestions = await env.FOOTBALL_DB.prepare(sql).bind(...params).all();
    return suggestions.results || [];
  } catch (error) {
    return [];
  }
  ...
  return [
    {
      ...
      function: {
        ...
        description: "Resolve football team name to canonical NFL team information with aliases and statistics",
        parameters: {
          type: "object",
          properties: {
            name: {
              type: "string",
              description: "Team name, city, or abbreviation (e.g., 'Chiefs', 'Kansas City', 'KC')"
            },
            fuzzy: {
              type: "boolean",
    ...
    },
    {
      ...
      function: {
        ...
        description: "Resolve football player name to canonical NFL player information with aliases and statistics",
        parameters: {
          type: "object",
          properties: {
            name: {
              type: "string",
              description: "Player name or nickname (e.g., 'Patrick Mahomes', 'Mahomes', 'Showtime')"
            },
            team: {
              type: "string",
    ...
    },
    {
      type: "function", 
      function: {
        name: "search_football_teams",
        description: "Search for NFL teams with filters and return multiple results",
        parameters: {
          type: "object",
          properties: {
            query: {
            ...
            division: {
              type: "string",
              description: "Filter by division (e.g., 'AFC East', 'NFC North')"
            },
            conference: {
              type: "string",
    ...
    },
    {
      type: "function",
      function: {
        name: "search_football_players", 
        description: "Search for NFL players with filters and return multiple results",
        parameters: {
          type: "object",
          properties: {
            query: {
            ...
            position: {
              type: "string",
              description: "Filter by position (e.g., 'QB', 'RB', 'WR', 'TE', 'K', 'DEF')"
            },
            active: {
              type: "boolean",
