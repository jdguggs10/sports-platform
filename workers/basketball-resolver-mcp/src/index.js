/**
 * Basketball Entity Resolver MCP Worker
 * 
 * Lightweight LLM resolver for basketball player and team naming discrepancies.
 * Resolves fuzzy entity names to canonical IDs for MCP tool calls.
 * 
 * Features:
 * - NBA team name resolution with aliases
...
export default {
  async fetch(request, env) {
    try {
      ...
      return new Response('Basketball Entity Resolver MCP v1.0', {
      ...
      return jsonResponse({
        error: error.message,
        service: 'basketball-resolver-mcp'
      }, 500);
    }
  }
};
...
async function handleHealthCheck(env) {
  try {
    ...
    const teamCount = await env.BASKETBALL_DB.prepare(
      'SELECT COUNT(*) as count FROM teams'
    ).first();
    
    const playerCount = await env.BASKETBALL_DB.prepare(
...
async function handleTeamSearch(body, env) {
  try {
    ...
    const teams = await env.BASKETBALL_DB.prepare(sql).bind(...params).all();
...
async function handlePlayerSearch(body, env) {
  try {
    ...
    const players = await env.BASKETBALL_DB.prepare(sql).bind(...params).all();
...
async function resolveTeam(name, env, options = {}) {
  try {
    ...
    let team = await env.BASKETBALL_DB.prepare(`
    ...
    // Try alias match
    if (!team) {
      team = await env.BASKETBALL_DB.prepare(`
    ...
    // Add aliases
    const aliases = await env.BASKETBALL_DB.prepare(`
    ...
    if (options.includeStats) {
      const stats = await env.BASKETBALL_DB.prepare(`
        SELECT 
...
async function resolvePlayer(name, env, options = {}) {
  try {
    ...
    let player = await env.BASKETBALL_DB.prepare(sql).bind(...params).first();
    ...
    if (!player) {
      ...
      player = await env.BASKETBALL_DB.prepare(sql).bind(...params).first();
    }
...
async function getTeamSuggestions(name, env, limit = 5) {
  try {
    const suggestions = await env.BASKETBALL_DB.prepare(`
...
async function getPlayerSuggestions(name, env, options = {}, limit = 5) {
  try {
    ...
    const suggestions = await env.BASKETBALL_DB.prepare(sql).bind(...params).all();
    return suggestions.results || [];
  } catch (error) {
    return [];
  }
  ...
  return [
    {
      ...
      function: {
        ...
        description: "Resolve basketball team name to canonical NBA team information with aliases and statistics",
        parameters: {
          type: "object",
          properties: {
            name: {
              type: "string",
              description: "Team name, city, or abbreviation (e.g., 'Lakers', 'Los Angeles', 'LAL')"
            },
            fuzzy: {
              type: "boolean",
    ...
    },
    {
      ...
      function: {
        ...
        description: "Resolve basketball player name to canonical NBA player information with aliases and statistics",
        parameters: {
          type: "object",
          properties: {
            name: {
              type: "string",
              description: "Player name or nickname (e.g., 'LeBron James', 'LeBron', 'King James')"
            },
            team: {
              type: "string",
    ...
    },
    {
      type: "function", 
      function: {
        name: "search_basketball_teams",
        description: "Search for NBA teams with filters and return multiple results",
        parameters: {
          type: "object",
          properties: {
            query: {
    ...
    },
    {
      type: "function",
      function: {
        name: "search_basketball_players", 
        description: "Search for NBA players with filters and return multiple results",
        parameters: {
          type: "object",
          properties: {
            query: {
